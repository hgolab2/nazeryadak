...
class Product extends Model
{
    ...
    private const DIGIT_MAP = [
        // ارقام فارسی (U+06F0..U+06F9) و عربی-هندی (U+0660..U+0669).
        // با کد نقطه نوشته می‌شوند نه با خودِ نویسه: نسخه‌ی قبلی نویسه‌ی خام
        // داشت و در یک بار ذخیره‌ی فایل با انکودینگ غلط دوباره انکود شد؛ از
        // آن به بعد هیچ رقم فارسی‌ای تطبیق نمی‌خورد و «پژو ۲۰۶» با «پژو 206»
        // دو عبارت جدا حساب می‌شد.
     ...
     *
     * تا پیش از این، اینجا یک قالب سه‌بندی بود که فقط نام و کد فنی را
     * جای‌گذاری می‌کرد: حدود ۴۳۰ کاراکتر، بدون یک کلمه اطلاعاتی که در
     * ستون‌های همان ردیف نباشد — دقیقا همان چیزی که ProductDescription در
     * سربرگش «محتوای نازک» می‌نامد و برای ۹۳۰ محصول بازنویسی‌اش کرد. ولی
     * مسیرِ تولیدِ محصولاتِ بی‌متن هنوز همان قالب قدیمی را می‌نوشت، پس
     * ۳۰۲۹ صفحه‌ی باقی‌مانده قرار بود دوباره با متن نازک پر شوند.
     *
     * حالا هر دو مسیر (دستور products:describe و دکمه‌ی «ساخت توضیحات» در
     * گزارش سلامت سئو) همان سازنده‌ای را صدا می‌زنند که products:seo-describe
     * استفاده می‌کند: مشخصات از ستون‌های خود محصول، نشانه‌های خرابی از دانش
     * دسته و تناسب خودرو از دانش پلتفرم — با برشِ چرخشی، تا دو محصولِ
     * هم‌دسته متن یکسان نگیرند.
     */
    public function generateDescription(): string
    {
        return \App\Support\ProductDescription::build($this);
    }
