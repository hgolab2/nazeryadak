...
/**
 * آدرس فایل ثابت به همراه شماره‌ی نسخه (زمان آخرین تغییر خود فایل).
 *
 * .htaccess روی css و js هدر «immutable» با عمر یک سال می‌گذارد و آدرس‌ها هم
 * بدون نسخه بودند؛ یعنی مرورگر کاربری که یک‌بار سایت را دیده، تا یک سال فایل
 * قدیمی را نگه می‌داشت و هیچ اصلاح ظاهری به او نمی‌رسید. سرویس‌ورکر هم همان
 * نسخه‌ی قدیمی را از کش سرو می‌کرد. با ?v=زمان‌تغییر، هر ویرایش فایل خودبه‌خود
 * یک آدرس تازه می‌سازد و کش دور زده می‌شود.
 *
 * فقط برای فایل‌های خودمان لازم است؛ بسته‌های وندور (بوت‌استرپ، جی‌کوئری…)
 * تغییر نمی‌کنند و بی‌نسخه می‌مانند تا کششان دست‌نخورده بماند.
 */
